...
async def _generate_all(sentences: list[Sentence], audio_dir: Path, settings: Settings) -> list[Path]:
    ...
    # Group sentences by chapter, preserving order
    chapters = [
        ...
        for i, (chapter_id, ch_sentences) in enumerate(chapters):
            if settings.tts_backend == "elevenlabs":
                paths = await asyncio.to_thread(
                    tts.generate_chapter, ch_sentences, chapter_id, audio_dir
                )
            else:
                # Kokoro/other: fall back to per-sentence generation
                paths = []
                for s in ch_sentences:
                    p = await asyncio.to_thread(tts.generate, s, audio_dir)
                    paths.append(p)
